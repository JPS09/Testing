...
Search
    ...
    Wait Until Keyword Succeeds    10s    300ms    Click Element    //button[contains(@class, "btn btn--reset form-icon js-search__icon")]
Click On Element
...
Click on Element with Ratings
    ${status}    Run Keyword And Return Status    Wait Until Element Is Visible    ${xpath_select_first_element_with_rating}    15  
    ...
    [Arguments]    ${first_list}    ${second_list}
        FOR    ${element_first_list}    IN    @{first_list}
            FOR    ${element_second_list}    IN    @{second_list}
                Should Be True    '${element_first_list}'=='${element_second_list}'    "${element_first_list}" is not equal to "${element_second_list}"
            END
        END      
