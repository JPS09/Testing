...
Test 2 - Utilisation de la recherche
    ...
    ...    locator=//div[contains(@class,'c-heading__title o-vstack gap-sm')]/h1/span
    ...    expected=Livre de poche
...
Test 9 - Se connecter si besoin
    Open and Maximise    https://www.cdiscount.com/
    Click On Element    button    title    Continuer sans accepter
    ${logged?}    Check if Logged In
    IF    '${logged?}' == '${False}'
        Log-In    ${cdiscount_email}    ${cdiscount_pwd}
        Check if Logged In
    END

Test 10 - Déjà Connecté
    Open and Maximise    https://www.cdiscount.com/
    Click On Element    button    title    Continuer sans accepter
    Log-In    ${cdiscount_email}    ${cdiscount_pwd}
    ${logged?}    Check if Logged In
